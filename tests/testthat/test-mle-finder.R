...
test_that("calculation of log likelihood gradient is correct", {
  ...
  analytical_ll_gradient <- log_likelihood_gradient(regression_coef = coef_true * 0.9, design, outcome,
    noise_var = 1)
  approx_ll_gradient <- approx_grad_via_finite_diff(function(regression_coef) log_likelihood_func(regression_coef,
...
test_that("newton and bfgs outputs coincide on logit model", {
  ...
  expect_true(are_all_close(coef(via_newton_out), coef(via_bfgs_out), abs_tol = 0.01, rel_tol = 0.01))
})
